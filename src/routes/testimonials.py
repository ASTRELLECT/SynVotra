...
async def get_testimonials(
    ...
    try:
        ...
        return {"status_code": 500, "detail": "An unexpected error occurred while fetching testimonials."}
...
async def get_testimonial(
    ...
    try:
        ...
        if not testimonial:
            ...
            return {"status_code": 404, "detail": "Testimonial not found."}
        ...
        if not current_user.is_admin and testimonial.user_id != current_user.id and testimonial.status != TestimonialStatus.APPROVED:
            ...
            return {"status_code": 403, "detail": "Not enough permissions to access this testimonial."}
        ...
        return {"status_code": 500, "detail": "An unexpected error occurred while accessing this testimonial."}
...
async def create_testimonial(
    ...
    try:
        ...
        return {"status_code": 400, "detail": "Could not create testimonial due to database constraint."}

    except Exception as e:
        db.rollback()
        logger.error(f"❌ Unexpected error creating testimonial: {str(e)}")
        return {"status_code": 500, "detail": "An unexpected error occurred while submitting testimonial."}
    ...
    try:
        ...
        # Regular user flow
            ...
            if db_testimonial.user_id != current_user.id:
                ...
                return {"status_code": 403, "detail": "Not authorized to update this testimonial."}
            ...
            if db_testimonial.status != TestimonialStatus.PENDING:
                ...
                return {"status_code": 400, "detail": "Only pending testimonials can be updated."}
            ...
            if "status" in update_dict or "admin_comments" in update_dict:
                ...
                return {"status_code": 403, "detail": "Regular users can only update content."}
        ...
        db.rollback()
        logger.error(f"🚫 Database integrity error: {str(e)}")
        return {"status_code": 400, "detail": "Could not update testimonial due to database constraint"}

    except Exception as e:
        db.rollback()
        logger.error(f"❌ Unexpected error updating testimonial: {str(e)}")
        return {"status_code": 500, "detail": "An unexpected error occurred while updating testimonial."}
    ...
    try:
        ...
        if not current_user.is_admin:
            if db_testimonial.user_id != current_user.id:
                ...
                return {"status_code": 403, "detail": "Not authorized to delete this testimonial."}

            if db_testimonial.status != TestimonialStatus.PENDING:
                logger.warning(f"🚫 400 - Cannot delete testimonial with status {db_testimonial.status}")
                return {"status_code": 400, "detail": "Only pending testimonials can be deleted."}
        ...
        return {"status_code": 500, "detail": "An unexpected error occurred while deleting the testimonial."}